...
For the PR 2365/2367 sort sites there is a CPU-count sweep: snapshot the serial-sorts build and the parallel-sorts build (binary plus klayout libraries) as `bench-before/` and `bench-after/` inside one bench root and run `./scripts/benchmark_pr_sorts.sh <bench-root> "1 2 4 8 10"`. It pins each run with `taskset` (TBB sizes its pool from the affinity mask) and runs after/before/after/before per CPU count to cancel drift. Plot with `uv run --script scripts/plot_pr_sort_scaling.py <bench-root> --output-dir pr-sort-plots`, which writes `speedup-vs-cpus.svg`, `runtime-vs-cpus.svg`, and a median table.
