...
private:
  ...
  bool do_process (Rec &rec, typename BoxConvertAdaptor::box_type::coord_type enl, const BoxConvertAdaptor &bc = BoxConvertAdaptor ())
  {
    ...
    if (m_pp.size () <= m_scanner_thr) {
      ...
      //  Tracks already-reported (canonicalized) object pairs to avoid duplicate
      //  reports across bands. Keyed on the larger of the two pointers; the value
      //  set holds the smaller ones. This replaces a std::set<pair> and turns the
      //  per-candidate membership test and the per-object purge below into hashed
      //  operations with better cache locality. The ordering of this structure
      //  never influenced output (reports are emitted in loop order), so this is
      //  behavior-preserving.
      ...
      while (future != m_pp.end ()) {
        ...
        while (cc != current) {
          ...
          //  Purge all pairs for which this object was the (canonical) key.
          seen.erase (cc->first);
          ++cc;
        }
...
private:
  ...
  bool do_process (Rec &rec, typename BoxConvertAdaptor1::box_type::coord_type enl, const BoxConvertAdaptor1 &bc1 = BoxConvertAdaptor1 (), const BoxConvertAdaptor2 &bc2 = BoxConvertAdaptor2 ())
  {
    ...
    if (m_pp1.empty () || m_pp2.empty ()) {
      ...
      //  Dedup of already-reported (Obj1,Obj2) pairs across bands, keyed on the
      //  Obj1 pointer (seen2 mirrors by Obj2 for symmetric per-object purging).
      //  Replaces std::set<pair>; ordering never affected output, so this is
      //  behavior-preserving but uses hashed lookups with better locality.
