...
InitHelper *InitHelper::ms_init = nullptr;
...
ref_matches (const char *name, const std::string &ref)
{
  if (!name) { // NOLINT(bugprone-branch-clone)
    return false;
  } else if (name == ref) { // NOLINT(bugprone-branch-clone)
    return true;
...
GitObject::read (const std::string &org_url, const std::string &org_filter, const std::string &subfolder, const std::string &branch, double  /*timeout*/, tl::InputHttpStreamCallback * /*callback*/)
{
  ...
  try {
    ...
    //  checkout
    checkout_branch (cloned_repo, remote, &checkout_opts, branch.empty () ? nullptr : branch.c_str ());
